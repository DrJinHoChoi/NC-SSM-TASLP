...
\caption{NC-SSM architecture. The horizontal signal path flows left-to-right: raw audio is optionally enhanced by spectral subtraction with adaptive bypass, then processed through STFT/mel extraction, Learned Spectral Gate (LSG), DualPCEN normalization, and two NC-SSM blocks with per-sub-band selectivity gates $\bm{\sigma}$. SNR estimation from mel features conditions LSG, the bypass gate, and the per-sub-band $\bm{\sigma}$ gates. Stationarity ($g_t$) and spectral flatness ($\beta_t$) provide additional noise-type-aware conditioning. Total: 7{,}443 parameters.}
...
\begin{table}[t]
\centering
\caption{Clean accuracy on GSC V2 (12-class). NC-SSM matches the CNN baseline with fewer parameters.}
\label{tab:clean}
\small
\begin{tabular}{lrr}
\toprule
...
NC-SSM-Large   & 10.2 & 1.15 & 2.4  & TBD  & TBD \\
\bottomrule
...
(3)~on-device validation on physical Cortex-M7 hardware;
