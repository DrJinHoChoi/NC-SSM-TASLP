...
\begin{table}[t]
\centering
\caption{Clean accuracy on GSC V2 (12-class). NC-SSM-Large surpasses the CNN baseline with 1.6$\times$ fewer full-pipeline MACs (4.0$\times$ model-only).}
\label{tab:clean}
\small
\begin{tabular}{lrr}
\toprule
...
\begin{table}[t]
\centering
\caption{Parameter scaling comparison: SSM vs.\ CNN. Full-pipeline MACs include preprocessing (STFT, mel, SNR estimation, PCEN/log-mel). NC-SSM achieves higher accuracy per MAC at every scale point.}
\label{tab:scaling}
\small
\setlength{\tabcolsep}{2.8pt}
...
\midrule
\multicolumn{7}{l}{\textit{SSM family (proposed)}} \\
NC-SSM         & 7.4  & 3.42 & 0.86 & 7.1  & 95.3 & \textbf{27.8} \\
NC-SSM-Large   & 10.2 & 3.80 & 1.24 & 7.9  & \textbf{95.6} & 25.2 \\
\bottomrule
