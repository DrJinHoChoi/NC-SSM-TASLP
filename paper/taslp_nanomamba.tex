...
\definecolor{plotnasg}{RGB}{170,85,210}
...
\thanks{This paper extends the authors' preliminary work presented at Interspeech 2026~\cite{choi2026nanomamba_interspeech}. Extensions include: (1)~formal analysis of SSM structural noise robustness (Section~\ref{sec:structural}), (2)~detailed noise propagation theory with variance bounds (Section~\ref{sec:analysis}), (3)~NC-SSM with per-sub-band selectivity, conditioning, LSG, and spectral enhancement (Section~\ref{sec:ncssm}), (4)~comprehensive evaluation with 6~models across 36~conditions (Section~\ref{sec:results}), (5)~Noise-Aware Selective Gating (NASG) with adaptive state masking achieving $O(\sigma_n^2)$ noise propagation (Section~\ref{sec:nasg}), and (6)~SNR teacher-student training framework (Section~\ref{sec:teacher_student}).}%
...
NC-SSM addresses this limitation through five complementary mechanisms: (1)~per-sub-band selectivity modulation that adapts SSM dynamics to local noise characteristics, (2)~noise-aware selective gating (NASG) that achieves $O(\sigma_n^2)$ noise propagation---matching the CNN bound---by eliminating residual selectivity at low SNR, (3)~adaptive state masking for SNR-dependent dimensionality reduction, (4)~a Learned Spectral Gate (LSG) for input-level denoising, and (5)~optimized spectral subtraction with adaptive bypass gating.
An SNR teacher-student training framework enables NASG to use oracle SNR during training while deploying with a lightweight spectral-flatness bridge.
...
Scaling to NC-SSM-Large (10{,}201 parameters) further improves clean accuracy to \textbf{95.6\%}, surpassing BC-ResNet-1 while maintaining \textbf{4.1$\times$ fewer MACs}.
Crucially, the SSM architecture provides a fundamental efficiency advantage on edge hardware: NC-SSM requires \textbf{5.5$\times$ fewer MACs} (0.85M vs.\ 4.70M), \textbf{5.4$\times$ lower latency} (1.8\,ms vs.\ 9.8\,ms on Cortex-M7), and \textbf{3.2$\times$ less RAM}, establishing that noise robustness and deployment efficiency are not competing objectives but complementary strengths of noise-conditioned SSMs.
\end{abstract}

\begin{IEEEkeywords}
Keyword spotting, state space model, noise robustness, selectivity modulation, noise-aware gating, teacher-student training, edge device
\end{IEEEkeywords}
...
\item \textbf{Noise-Aware Selective Gating (NASG)}: An SNR-dependent gate eliminates residual selectivity at low SNR, achieving $O(\sigma_n^2)$ noise propagation---matching the CNN bound---with only 2 parameters per block.
\item \textbf{Adaptive state masking}: Input-gated per-state masks reduce the effective state dimensionality under noise, preventing noise entry into higher-indexed states with longer memory.
...
\end{enumerate}
An \textbf{SNR teacher-student training framework} enables NASG and state masking to learn from oracle SNR during training while deploying with a lightweight spectral-flatness bridge at inference.
...
The decisive advantage is computational: NC-SSM requires \textbf{5.5$\times$ fewer MACs}, \textbf{5.4$\times$ lower latency} (1.8\,ms vs.\ 9.8\,ms on Cortex-M7), and \textbf{3.2$\times$ less RAM}, establishing that noise robustness and deployment efficiency are complementary strengths of noise-conditioned SSMs.
...
\item \textbf{Noise-Aware Selective Gating (NASG)}: Formally bridges the $O(\sigma_n^6)$-to-$O(\sigma_n^2)$ gap with only 2 parameters per block, combined with input-gated state masking for adaptive state dimensionality reduction (Section~\ref{sec:nasg}).
\item \textbf{SNR teacher-student training}: Oracle-supervised training with a spectral-flatness bridge for deployment, enabling NASG without runtime oracle SNR (Section~\ref{sec:teacher_student}).
\item \textbf{Efficiency--robustness Pareto front}: NC-SSM achieves 5.5$\times$ fewer MACs and 5.4$\times$ lower latency than the CNN baseline at matched accuracy with only 7{,}453 parameters, demonstrating that noise robustness and deployment efficiency are complementary (Sections~\ref{sec:results}--\ref{sec:mcu_deploy}).
\end{enumerate}
...
NanoMamba requires only +10.6\,\%p from noise-aug to reach its best performance; BC-ResNet-1 needs +20.7\,\%p (2.0$\times$) and DS-CNN-S needs +38.8\,\%p (3.7$\times$)---confirming that CNNs are \emph{almost entirely dependent} on training data for noise robustness.
...
The remaining sections realize this principle architecturally via selectivity modulation (Sections~\ref{sec:smssm}--\ref{sec:nasg}).
...
NC-SSM extends SM-SSM with frequency-aware noise conditioning, noise-aware selective gating (NASG), adaptive state masking, and an SNR teacher-student training framework.
...
\subsection{Noise-Aware Selective Gating (NASG)}
\label{sec:nasg}

The selectivity gate $\sigma$ (Section~\ref{sec:smssm}) reduces selectivity at low SNR, but residual selectivity ($\sigma \approx 0.05$ at $-15$\,dB) still introduces multiplicative coupling: even $\sigma = 0.05$ amplifies noise cross-terms by $\sigma_n^4 \approx 1{,}000\times$ at $-15$\,dB.
NASG introduces a second, SNR-dependent gate that \emph{eliminates} residual selectivity entirely.

Let $h_{\text{snr}} \in [-1, 1]$ denote an SNR hint signal (Section~\ref{sec:teacher_student}).
The NASG gate is:
\begin{equation}
g = \sigma\!\big(\alpha_{\text{nasg}} \cdot h_{\text{snr}} + \beta_{\text{nasg}}\big),
\label{eq:nasg_gate}
\end{equation}
where $\alpha_{\text{nasg}}$ and $\beta_{\text{nasg}}$ are learnable parameters (initialized to 5.0 and 0.0 respectively).
The modified blending becomes:
\begin{equation}
\Delta_t = \sigma_\Delta \cdot g \cdot \Delta_{\text{sel}}(x_t) + (1 - \sigma_\Delta) \cdot \theta_\Delta,
\label{eq:nasg_blend}
\end{equation}
and similarly for $\mathbf{B}_t$, $\mathbf{C}_t$.
In practice, this is implemented by scaling the projection input: $\tilde{x}_t = g \cdot x_t$ before computing $\Delta_{\text{sel}}(\tilde{x}_t)$, $\mathbf{B}_{\text{sel}}(\tilde{x}_t)$, and $\mathbf{C}_{\text{sel}}(\tilde{x}_t)$---mathematically equivalent but computationally simpler.

\begin{proposition}[O($\sigma_n^2$) Guarantee]
\label{prop:nasg}
With NASG, $\mathrm{Var}[u_t] = O(\sigma_n^2)$ at all SNR levels.
\end{proposition}

\begin{proof}
At low SNR, $g \to 0$: the selective components $\Delta_{\text{sel}}, \mathbf{B}_{\text{sel}}, \mathbf{C}_{\text{sel}}$ vanish, and the state update reduces to $u_t = \theta_\Delta \cdot \theta_B \cdot x_t$---a fixed linear transform with $O(\sigma_n^2)$ variance.
At high SNR, $g \to 1$: full selectivity is preserved, and noise variance is negligible.
The transition is smooth via the sigmoid gate.
\end{proof}

Experimentally, the effective selectivity $\sigma \cdot g \approx 0.001$ at $-15$\,dB, suppressing noise cross-terms by $\sim$2{,}500$\times$ compared to the $\sigma$-gate alone.
NASG adds only \textbf{2 parameters per block} ($\alpha_{\text{nasg}}$, $\beta_{\text{nasg}}$).

\subsection{Adaptive State Masking}
\label{sec:state_mask}

While NASG suppresses the \emph{selective} component, the fixed parameters $\theta_\Delta$, $\bm{\theta}_B$ still drive the state update in LTI mode.
At low SNR, noise accumulates across all $N$ state dimensions, with higher-indexed states (longer effective memory via $|\bar{A}_n| \to 1$) accumulating proportionally more noise.
We introduce \emph{input-gated state masking}:
\begin{equation}
\mathbf{h}_t = \bar{\mathbf{A}}_t\,\mathbf{h}_{t-1} + \mathbf{m} \odot \bar{\mathbf{B}}_t\,x_t,
\label{eq:state_mask}
\end{equation}
where the per-state mask is:
\begin{equation}
m_n = \sigma\!\big(\gamma_m \cdot h_{\text{snr}} + \beta_m - p_n\big), \quad p_n = \frac{2n}{N{-}1},
\label{eq:state_mask_gate}
\end{equation}
with learnable scale $\gamma_m$ (init 3.0) and bias $\beta_m$ (init 0.0).
The linearly increasing penalty $p_n$ prioritizes deactivation of higher-indexed states, which have longer memory and accumulate more noise.

Crucially, the mask is applied to the state \emph{input} rather than the output.
Output-gating ($y_t = (\mathbf{m} \odot \mathbf{C})\,\mathbf{h}_t$) allows noise to accumulate freely across $T$ frames before discarding the corrupted state; input-gating prevents noise \emph{entry}, keeping $\|\mathbf{h}_{t,n}\| \approx 0$ for masked states.
At $-15$\,dB, the effective state dimension reduces to $N_{\text{eff}} \approx 0.2\text{--}0.3$ (from $N{=}8$), concentrating computation on the most informative, least noisy state components.
State masking adds \textbf{2 parameters per block} ($\gamma_m$, $\beta_m$).

\subsection{SNR Teacher-Student Training}
\label{sec:teacher_student}

NASG and state masking require an SNR hint $h_{\text{snr}} \in [-1, 1]$.
During training, the oracle SNR is known (used for noise mixing); during inference, it must be estimated.
We adopt a teacher-student paradigm.

\smallskip\noindent\textbf{SNR hint mapping.}
The oracle SNR is compressed to $[-1, 1]$ via:
\begin{equation}
h_{\text{snr}} = \tanh\!\Big(\frac{\text{SNR}_{\text{dB}}}{10}\Big).
\label{eq:snr_hint}
\end{equation}
Clean speech uses $\text{SNR}_{\text{dB}} = 25$, yielding $h_{\text{snr}} \approx 0.987$.
This mapping provides approximately linear sensitivity in the critical $-10$ to $+10$\,dB range while saturating at extremes.

\smallskip\noindent\textbf{Teacher (training).}
The oracle $h_{\text{snr}}$ is passed directly to the NASG gate~\eqref{eq:nasg_gate} and state mask~\eqref{eq:state_mask_gate}, enabling the model to learn the theoretically optimal gate response as a function of true SNR.

\smallskip\noindent\textbf{Student (inference).}
At deployment, we estimate $h_{\text{snr}}$ from spectral flatness (SF)---the ratio of the geometric to arithmetic mean of mel-band energies:
\begin{equation}
\hat{h}_{\text{snr}} = \tanh\!\big(w_{\text{sf}} \cdot \overline{\text{SF}} + b_{\text{sf}}\big),
\label{eq:sf_bridge}
\end{equation}
where $w_{\text{sf}}$ and $b_{\text{sf}}$ are learnable parameters trained via a detached auxiliary loss:
\begin{equation}
\mathcal{L}_{\text{sf}} = \lambda_{\text{sf}} \cdot \text{MSE}\!\big(\hat{h}_{\text{snr}},\; h_{\text{snr}}^{\text{oracle}}\big), \quad \lambda_{\text{sf}} = 0.5.
\label{eq:sf_loss}
\end{equation}
The \texttt{detach()} operation on the mel spectrogram blocks gradient flow from the SF bridge to the feature extractor, ensuring only $w_{\text{sf}}$ and $b_{\text{sf}}$ receive gradients from $\mathcal{L}_{\text{sf}}$.

The teacher-student framework adds \textbf{2 model-level parameters} ($w_{\text{sf}}$, $b_{\text{sf}}$).

\subsection{NC-SSM Parameter Summary}

NC-SSM adds per block: $\mathbf{w}^{\text{sub}}$ ($N$), $\alpha_s$ (1), $\mathbf{w}_{sf}$ ($N$) = $2N{+}1 = 13$ for frequency conditioning; $\alpha_{\text{nasg}}$, $\beta_{\text{nasg}}$ (2) for NASG; $\gamma_m$, $\beta_m$ (2) for state masking.
Total per block: $2N{+}5 = 17$ parameters.
With 2 blocks: +34.
LSG: +120.
SF bridge: +2.
\textbf{Total NC-SSM: 7{,}453 parameters}---11 \emph{fewer} than BC-ResNet-1 (7{,}464).

Warm-start initialization preserves SM-SSM behavior: $\mathbf{w}^{\text{sub}} = 5.0 \cdot \mathbf{1}_N$, $\alpha_s = 0$, $\mathbf{w}_{sf} = \mathbf{0}$, $\alpha_{\text{nasg}} = 5.0$, $\beta_{\text{nasg}} = 0$, $\gamma_m = 3.0$, $\beta_m = 0$.
...
\node[ssm, minimum width=2.4cm, minimum height=0.7cm] at ([yshift=0.6cm]blk1box.center) (scan1) {Selective Scan\\{\tiny $d{=}16,\,N{=}6$}};
\node[ncnew, minimum width=2.4cm, minimum height=0.7cm] at ([yshift=-0.25cm]blk1box.center) (sig1) {$\bm{\sigma}$ + NASG\\{\tiny per-sub-band}};
\node[ncnew, minimum width=2.4cm, minimum height=0.5cm, font=\tiny\sffamily] at ([yshift=-1.05cm]blk1box.center) (mask1) {State Mask\\{\tiny input-gated}};
...
\node[ssm, minimum width=2.4cm, minimum height=0.7cm] at ([yshift=0.6cm]blk2box.center) (scan2) {Selective Scan\\{\tiny $d{=}16,\,N{=}6$}};
\node[ncnew, minimum width=2.4cm, minimum height=0.7cm] at ([yshift=-0.25cm]blk2box.center) (sig2) {$\bm{\sigma}$ + NASG\\{\tiny per-sub-band}};
\node[ncnew, minimum width=2.4cm, minimum height=0.5cm, font=\tiny\sffamily] at ([yshift=-1.05cm]blk2box.center) (mask2) {State Mask\\{\tiny input-gated}};
...
\draw[arr, color=newcolor!80] (mask1) -- (scan1);
\draw[arr, color=newcolor!80] (mask2) -- (scan2);
...
% Teacher/SF Bridge
\node[ncnew, below=0.3cm of sf, minimum width=1.8cm, minimum height=0.6cm, font=\tiny\sffamily] (sfbridge) {SF Bridge\\{\tiny Teacher/Student}};
...
% SF Bridge → NASG/State Mask conditioning
\draw[cond] (sf) -- (sfbridge);
\draw[cond] (sfbridge.east) -- ++(0.3,0) |- ([yshift=-0.1cm]mask1.south);
\draw[cond] ([xshift=0.3cm]sfbridge.east) -- ++(0.3,0) |- ([yshift=-0.1cm]mask2.south);
...
\caption{NC-SSM architecture: Noise-conditioned SSM with per-sub-band selectivity modulation, NASG, adaptive state masking, SNR teacher-student bridge, Learned Spectral Gate, and optimized spectral enhancement---the most parameter-efficient noise-robust KWS model at 7{,}453 parameters.}
...
\begin{table}[t]
\centering
\caption{Model configurations. All primary models are within BC-ResNet-1's 7{,}464-parameter budget; NC-SSM-Large scales beyond this budget to surpass CNN accuracy. NC-SSM achieves the fewest parameters among SSM variants while adding noise-adaptive capabilities.}
\label{tab:configs}
\small
...
NC-SSM+NanoSE & SSM+LSG+SE & 11{,}883 & 11.6\,KB \\
NC-SSM-L+NASG & SSM+LSG+NASG & 10{,}211 & 10.0\,KB \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:configs} lists all models.
\textbf{NM-Matched} (NanoMamba-Matched) denotes the base SSM model---SA-SSM v2 with selectivity-aware gating---configured to match BC-ResNet-1's parameter budget ($d_{\text{model}}{=}20$, $d_{\text{state}}{=}6$, $d_{\text{conv}}{=}3$, expand${=}1.5$; 7{,}402 params).
It serves as the SSM baseline against which NC-SSM's noise-conditioning components are evaluated.
The SSM progression (NM-Matched $\to$ SM-SSM $\to$ NC-SSM) demonstrates incremental improvements with minimal parameter additions.
NC-SSM-Large scales capacity via $d_{\text{model}}{=}24$, $N{=}8$ (vs.\ 16, 6), surpassing BC-ResNet-1's accuracy while maintaining $>$4$\times$ efficiency advantage.
...
\textbf{SNR teacher-student training} (Section~\ref{sec:teacher_student}): During noise-augmented training, the oracle SNR hint $h_{\text{snr}} = \tanh(\text{SNR}_{\text{dB}}/10)$ is passed directly to NASG and state masking gates.
Clean batches use $h_{\text{snr}} = \tanh(25/10) \approx 0.987$.
The SF bridge auxiliary loss ($\lambda_{\text{sf}} = 0.5$, MSE with \texttt{detach()}) trains the inference-time SNR estimator jointly with the main classification loss, without modifying the four-phase noise curriculum.
...
NC-SSM & 7{,}453 & 95.3 \\
NC-SSM-Large & 10{,}201 & 95.6 \\
NC-SSM+NanoSE & 11{,}883 & \textbf{95.9} \\
NC-SSM-L+NASG & 10{,}211 & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table*}[t]
\centering
\caption{Noise robustness comparison (accuracy \%). All models trained with noise augmentation. Best per condition in \textbf{bold} (excluding the $3.2\times$ larger DS-CNN-S). NC-SSM-L+NASG results pending.}
...
\midrule
\multirow{7}{*}{Factory}
  & DS-CNN-S (23.8K)       & 96.4 & 63.8 & 70.4 & 79.9 & 90.4 & 93.5 & 94.9 & 95.7 \\
  & BC-ResNet-1 (7.5K)     & 95.0 & \textbf{63.2} & \textbf{71.7} & \textbf{81.5} & 88.0 & 90.7 & 92.0 & 93.3 \\
  & NM-Matched (7.4K)      & 94.8 & 60.7 & 65.7 & 76.5 & 85.6 & 90.2 & 92.4 & 93.1 \\
  & NC-SSM (7.5K)          & 95.3 & 61.4 & 69.2 & 77.8 & 86.8 & 90.1 & 92.4 & 92.9 \\
  & NC-SSM-Large (10.2K)   & 95.6 & 61.3 & 68.6 & 79.1 & 87.0 & 91.0 & 93.1 & 93.4 \\
  & NC-SSM+NanoSE (11.9K)  & \textbf{95.9} & 60.7 & 68.9 & 80.7 & \textbf{89.3} & \textbf{92.6} & \textbf{94.0} & \textbf{94.6} \\
  & NC-SSM-L+NASG (10.2K)  & --- & --- & --- & --- & --- & --- & --- & --- \\
\midrule
\multirow{7}{*}{White}
  & DS-CNN-S (23.8K)       & 96.4 & 61.6 & 64.0 & 76.8 & 88.8 & 92.6 & 94.3 & 95.6 \\
  & BC-ResNet-1 (7.5K)     & 95.0 & \textbf{54.4} & 60.6 & 73.7 & 83.0 & 89.4 & 91.7 & 93.6 \\
  & NM-Matched (7.4K)      & 94.8 & 28.0 & 53.6 & 72.7 & 83.9 & 89.6 & 92.3 & 93.3 \\
  & NC-SSM (7.5K)          & 95.3 & 50.8 & \textbf{63.0} & 75.5 & 85.2 & 90.0 & 92.5 & 93.5 \\
  & NC-SSM-Large (10.2K)   & 95.6 & 34.1 & 59.8 & 76.5 & 86.4 & 91.0 & 93.4 & 94.4 \\
  & NC-SSM+NanoSE (11.9K)  & \textbf{95.9} & 40.4 & 59.8 & \textbf{79.2} & \textbf{87.5} & \textbf{91.6} & \textbf{93.6} & \textbf{94.7} \\
  & NC-SSM-L+NASG (10.2K)  & --- & --- & --- & --- & --- & --- & --- & --- \\
\midrule
\multirow{7}{*}{Babble}
  & DS-CNN-S (23.8K)       & 96.4 & 72.8 & 80.5 & 87.1 & 91.6 & 94.8 & 95.8 & 96.0 \\
  & BC-ResNet-1 (7.5K)     & 95.0 & 72.0 & \textbf{81.1} & 86.7 & 91.2 & 93.0 & 94.2 & 94.5 \\
  & NM-Matched (7.4K)      & 94.7 & 71.1 & 78.5 & 84.7 & 89.6 & 92.5 & 93.6 & 94.3 \\
  & NC-SSM (7.5K)          & 95.2 & 73.4 & 79.1 & 84.1 & 89.9 & 92.4 & 94.0 & 94.8 \\
  & NC-SSM-Large (10.2K)   & 95.5 & 74.0 & 80.2 & 86.8 & 91.3 & 93.3 & 94.4 & 95.1 \\
  & NC-SSM+NanoSE (11.9K)  & \textbf{95.9} & \textbf{74.1} & 80.8 & \textbf{88.1} & \textbf{91.6} & \textbf{93.7} & \textbf{94.8} & \textbf{95.4} \\
  & NC-SSM-L+NASG (10.2K)  & --- & --- & --- & --- & --- & --- & --- & --- \\
\midrule
\multirow{7}{*}{Street}
  & DS-CNN-S (23.8K)       & 96.4 & 61.5 & 67.1 & 78.1 & 88.1 & 92.5 & 94.9 & 95.8 \\
  & BC-ResNet-1 (7.5K)     & 95.0 & \textbf{60.5} & 63.3 & 73.3 & 83.2 & 87.9 & 92.1 & 93.4 \\
  & NM-Matched (7.4K)      & 94.7 & 52.6 & 62.5 & 72.6 & 84.0 & 89.4 & 91.8 & 93.3 \\
  & NC-SSM (7.5K)          & 95.2 & 60.2 & 64.3 & 75.2 & 82.2 & 88.9 & 92.2 & 93.7 \\
  & NC-SSM-Large (10.2K)   & 95.6 & 57.8 & 63.5 & 75.7 & 83.4 & 90.1 & 92.8 & 94.5 \\
  & NC-SSM+NanoSE (11.9K)  & \textbf{95.9} & 54.5 & \textbf{65.4} & \textbf{76.5} & \textbf{85.5} & \textbf{91.2} & \textbf{93.5} & \textbf{95.0} \\
  & NC-SSM-L+NASG (10.2K)  & --- & --- & --- & --- & --- & --- & --- & --- \\
\midrule
\multirow{7}{*}{Pink}
  & DS-CNN-S (23.8K)       & 96.4 & 61.3 & 62.7 & 76.2 & 88.3 & 93.1 & 95.1 & 96.0 \\
  & BC-ResNet-1 (7.5K)     & 95.0 & 54.9 & 61.9 & 75.8 & 85.6 & 90.8 & 93.3 & 94.2 \\
  & NM-Matched (7.4K)      & 94.7 & 17.7 & 42.7 & 73.6 & 85.2 & 91.0 & 93.2 & 94.1 \\
  & NC-SSM (7.5K)          & 95.2 & 55.2 & \textbf{62.9} & 76.7 & 86.6 & 91.5 & 93.0 & 94.4 \\
  & NC-SSM-Large (10.2K)   & 95.6 & \textbf{55.4} & 62.2 & 76.4 & 87.2 & 91.9 & 93.8 & 94.7 \\
  & NC-SSM+NanoSE (11.9K)  & \textbf{95.9} & 47.2 & 59.4 & \textbf{79.5} & \textbf{88.4} & \textbf{92.4} & \textbf{94.6} & \textbf{95.3} \\
  & NC-SSM-L+NASG (10.2K)  & --- & --- & --- & --- & --- & --- & --- & --- \\
\bottomrule
\end{tabular}
\end{table*}
...
\pgfplotsset{
  noise plot/.style={
    ...
    xmin=-17, xmax=17, ymin=25, ymax=100,
...
\begin{tikzpicture}
% Row 1: Factory, White, Babble
\begin{axis}[noise plot, name=p1,
    title={Factory}, ylabel={Accuracy (\%)}, xlabel={}]
\addplot[color=plotcnn, mark=square*]
    coordinates {(-15,64.5)(-10,74.8)(-5,83.5)(0,89.0)(5,91.9)(10,92.9)(15,93.9)};
\addplot[color=plotssm, mark=*]
    coordinates {(-15,61.4)(-10,69.2)(-5,77.8)(0,86.8)(5,90.1)(10,92.4)(15,92.9)};
\addplot[color=plotlarge, densely dashed, mark=diamond*, mark options={solid}]
    coordinates {(-15,61.3)(-10,68.6)(-5,79.1)(0,87.0)(5,91.0)(10,93.1)(15,93.4)};
\end{axis}

\begin{axis}[noise plot, name=p2, at={(p1.east)}, anchor=west, xshift=0.15cm,
    title={White}, ylabel={}, xlabel={}]
\addplot[color=plotcnn, mark=square*]
    coordinates {(-15,58.6)(-10,64.6)(-5,76.7)(0,85.9)(5,90.2)(10,92.6)(15,93.8)};
\addplot[color=plotssm, mark=*]
    coordinates {(-15,50.8)(-10,63.0)(-5,75.5)(0,85.2)(5,90.0)(10,92.5)(15,93.5)};
\addplot[color=plotlarge, densely dashed, mark=diamond*, mark options={solid}]
    coordinates {(-15,34.1)(-10,59.8)(-5,76.5)(0,86.4)(5,91.0)(10,93.4)(15,94.4)};
% Gap annotation at -15 dB: BC-ResNet-1 (58.6) vs NC-SSM-L (34.1)
\draw[<->, gray!60!black, line width=0.7pt]
    (axis cs:-16,35.5) -- (axis cs:-16,57.2);
\node[left, font=\tiny, text=gray!50!black, align=right]
    at (axis cs:-16.5,46) {24.5\,\%p};
\end{axis}

\begin{axis}[noise plot, name=p3, at={(p2.east)}, anchor=west, xshift=0.15cm,
    title={Babble}, ylabel={}, xlabel={}]
\addplot[color=plotcnn, mark=square*]
    coordinates {(-15,76.7)(-10,84.3)(-5,88.8)(0,92.1)(5,93.8)(10,94.4)(15,95.0)};
\addplot[color=plotssm, mark=*]
    coordinates {(-15,73.4)(-10,79.1)(-5,84.1)(0,89.9)(5,92.4)(10,94.0)(15,94.8)};
\addplot[color=plotlarge, densely dashed, mark=diamond*, mark options={solid}]
    coordinates {(-15,74.0)(-10,80.2)(-5,86.8)(0,91.3)(5,93.3)(10,94.4)(15,95.1)};
\end{axis}

% Row 2: Street, Pink (centered)
\begin{axis}[noise plot, name=p4,
    at={($(p1.south west)!0.5!(p2.south west)$)}, anchor=north west,
    yshift=-1.0cm,
    title={Street}, ylabel={Accuracy (\%)}, xlabel={SNR (dB)}]
\addplot[color=plotcnn, mark=square*]
    coordinates {(-15,60.7)(-10,65.1)(-5,77.7)(0,84.8)(5,90.1)(10,93.0)(15,94.1)};
\addplot[color=plotssm, mark=*]
    coordinates {(-15,60.2)(-10,64.3)(-5,75.2)(0,82.2)(5,88.9)(10,92.2)(15,93.7)};
\addplot[color=plotlarge, densely dashed, mark=diamond*, mark options={solid}]
    coordinates {(-15,57.8)(-10,63.5)(-5,75.7)(0,83.4)(5,90.1)(10,92.8)(15,94.5)};
\end{axis}

\begin{axis}[noise plot, name=p5, at={(p4.east)}, anchor=west, xshift=0.15cm,
    title={Pink}, ylabel={}, xlabel={SNR (dB)},
    legend to name=shared_legend,
    legend style={font=\scriptsize, legend columns=-1, column sep=8pt,
      draw=none}]
\addplot[color=plotcnn, mark=square*]
    coordinates {(-15,58.9)(-10,66.0)(-5,79.7)(0,88.4)(5,91.8)(10,93.8)(15,94.5)};
\addlegendentry{BC-ResNet-1 (7.5K\,/\,4.70M)}
\addplot[color=plotssm, mark=*]
    coordinates {(-15,55.2)(-10,62.9)(-5,76.7)(0,86.6)(5,91.5)(10,93.0)(15,94.4)};
\addlegendentry{NC-SSM (7.4K\,/\,0.85M)}
\addplot[color=plotlarge, densely dashed, mark=diamond*, mark options={solid}]
    coordinates {(-15,55.4)(-10,62.2)(-5,76.4)(0,87.2)(5,91.9)(10,93.8)(15,94.7)};
\addlegendentry{NC-SSM-L (10.2K\,/\,1.15M)}
\end{axis}

% Shared legend centered below all plots (below xlabel text)
\node[anchor=north] at ($(p4.south)!0.5!(p5.south) + (0,-0.9cm)$)
  {\ref{shared_legend}};

\end{tikzpicture}
\caption{Accuracy vs.\ SNR per noise type. NC-SSM-Large ($\blacklozenge$) matches BC-ResNet-1 ($\blacksquare$) above 5\,dB with 4.1$\times$ fewer MACs. The gap below $-5$\,dB reflects $O(\sigma_n^6)$ multiplicative noise propagation, which NASG is designed to close. NC-SSM-L+NASG curves to be added after experimental evaluation.}
\label{fig:snr_curve}
\end{figure*}
...
\midrule
NM-Matched  & +0.9 & \textbf{+19.9} & $-$1.6 & \textbf{+7.8} & \textbf{+4.1} \\
NC-SSM      & +0.7 & +15.4 & +0.4 & +1.4 & \textbf{+4.1} \\
NC-SSM+NanoSE & +2.3 & +17.4 & \textbf{+2.7} & +2.8 & $-$7.8 \\
...
SSMs benefit dramatically from SS under stationary noise: NM-Matched gains +19.9\,\%p (white) at $-15$\,dB, while CNN baselines improve by at most 3.7\,\%p.
This asymmetry follows from Section~\ref{sec:mult_noise}: SS reduces SSM internal variance by $\alpha^6$ vs.\ $\alpha^2$ for CNNs.
Notably, NC-SSM+NanoSE exhibits negative SS interaction for pink noise ($-7.8$\,\%p): NanoSE's learned mel-domain mask conflicts with SS's spectral subtraction, causing double suppression in the low-frequency bands where pink noise concentrates.
...
\begin{table}[t]
\centering
\caption{Cumulative ablation on factory noise. $\Delta$P: additional parameters vs.\ base model.
White $-15$\,dB shown as the most challenging condition for SSMs.}
\label{tab:ablation}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{lrccc}
...
\midrule
Base SA-SSM v2 (NM-Matched)     & 0   & 95.1 & 88.5 & 31.1 \\
\quad + SM-SSM ($\sigma$-gate)   & +38 & 95.2 & 88.2 & --- \\
\quad + NC-SSM conditioning      & +26 & 95.3 & 88.0 & 50.8 \\
\quad + NASG                     & +4  & ---  & ---  & --- \\
\quad + State masking            & +4  & ---  & ---  & --- \\
\quad + Teacher-student (SF)     & +2  & ---  & ---  & --- \\
\quad + LSG                      & +120 & --- & ---  & --- \\
\quad + SS+Bypass                & +0  & ---  & ---  & --- \\
\midrule
\textbf{NC-SSM (full)}          & \textbf{+51} & \textbf{---} & \textbf{---} & \textbf{---} \\
\bottomrule
\end{tabular}
\\[2pt]
{\footnotesize \textit{Note: --- entries pending experimental evaluation.}}
\end{table}
...
NASG (Section~\ref{sec:nasg}) resolves this gap.
...
While the $\sigma$-gate alone leaves residual selectivity ($\sigma \approx 0.05$ at $-15$\,dB), the NASG gate drives the effective selectivity to $\sigma \cdot g \approx 0.001$---a $50\times$ further reduction that eliminates multiplicative coupling entirely.

At $-15$\,dB, the combined selectivity $\sigma \cdot g \approx 0.001$ suppresses cross-term noise by ${\sim}2{,}500\times$, while adaptive state masking reduces effective state dimension to $N_{\text{eff}} \approx 0.2$--$0.3$.
...
\textbf{Model} & \textbf{Params} & \textbf{MACs} & \textbf{Lat.} & \textbf{Energy} & \textbf{RAM} \\
               &                & (M)          & (ms)         & ($\mu$J)       & (KB)        \\
\midrule
NM-Matched     & 7{,}402  & \textbf{0.83} & \textbf{1.7}  & \textbf{346}   & \textbf{30.5}  \\
NC-SSM         & 7{,}453  & 0.85 & 1.8  & 354   & 31.7  \\
NC-SSM-Large   & 10{,}201 & 1.15 & 2.4  & 479   & 38.2  \\
\midrule
BC-ResNet-1    & 7{,}464  & 4.70 & 9.8  & 1{,}958 & 102.0 \\
DS-CNN-S       & 23{,}756 & 24.5 & 51.0 & 10{,}203 & 285.7 \\
\midrule
\multicolumn{2}{l}{\textbf{NM / BC-ResNet-1}} & \textbf{5.7$\times$} & \textbf{5.8$\times$} & \textbf{5.7$\times$} & \textbf{3.3$\times$} \\
...
NM-Matched provides the closest parameter-matched comparison: 7{,}402 vs.\ BC-ResNet-1's 7{,}464 (0.8\% difference), achieving \textbf{5.7$\times$ fewer MACs}, \textbf{5.8$\times$ lower latency}, and \textbf{5.7$\times$ less energy}.
Notably, NM-Matched uses only DualPCEN~v2 and SA-SSM~v2 without NC-SSM's noise conditioning or LSG---demonstrating that the efficiency advantage is \emph{architectural}, not contingent on noise-specific modules.
NC-SSM further achieves \textbf{5.5$\times$ fewer MACs} with additional noise-conditioned selectivity.
...
NC-SSM (7{,}453 params) further improves to 354\,$\mu$J and 0.35\,mW average, yielding \textbf{5.5$\times$} battery advantage.
...
\smallskip\noindent\textbf{SF bridge improvement.}
The current spectral-flatness bridge (Section~\ref{sec:teacher_student}) measures spectral \emph{shape} rather than energy level, limiting its ability to distinguish different SNR levels of the same noise type.
An SS-based bridge using the average spectral subtraction gain---which monotonically relates to input SNR---is a natural improvement that could provide more accurate SNR estimation at inference time.
...
NC-SSM resolves this through per-sub-band selectivity modulation, Learned Spectral Gate, and optimized spectral subtraction with adaptive bypass.
Crucially, Noise-Aware Selective Gating (NASG) formally bridges the $O(\sigma_n^6)$-to-$O(\sigma_n^2)$ gap: the effective selectivity $\sigma \cdot g \approx 0.001$ at $-15$\,dB suppresses noise cross-terms by $\sim$2{,}500$\times$, while adaptive state masking reduces the effective state dimensionality to prevent noise accumulation.
An SNR teacher-student framework enables oracle-supervised training with a lightweight spectral-flatness bridge for deployment.
NC-SSM achieves \textbf{7{,}453 parameters}---11 fewer than BC-ResNet-1 (7{,}464)---while matching its clean accuracy (95.0\%) with \textbf{5.5$\times$ fewer MACs} and \textbf{1.8\,ms latency} on Cortex-M7.
Scaling to NC-SSM-Large further improves clean accuracy to \textbf{95.6\%}---surpassing BC-ResNet-1---while maintaining \textbf{4.1$\times$ fewer MACs}, \textbf{2.4\,ms latency}, and \textbf{2.7$\times$ less RAM}.
...
The complete training and evaluation code is available at \url{https://github.com/DrJinHoChoi/NC-SSM-TASLP}.
