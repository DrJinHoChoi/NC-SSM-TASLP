...
% Plot colors: 3-model distinct palette for Figure 2
...
\item \textbf{Efficiency--robustness Pareto front}: Comprehensive evaluation showing NC-SSM achieves 5.5$\times$ fewer MACs and 5.4$\times$ lower latency than the CNN baseline at matched accuracy, demonstrating that the two objectives are complementary rather than competing (Sections~\ref{sec:results}--\ref{sec:mcu_deploy}).
\end{enumerate}
...
NC-SSM & 7{,}443 & 95.3 \\
NC-SSM-Large & 10{,}201 & \textbf{95.6} \\
\bottomrule
\end{tabular}
\end{table}
...
\multirow{5}{*}{Factory}
  ...
  & BC-ResNet-1 (7.5K)     & 95.3 & \textbf{64.5} & \textbf{74.8} & \textbf{83.5} & \textbf{89.0} & \textbf{91.9} & 92.9 & \textbf{93.9} \\
  & NM-Matched (7.4K)      & 95.1 & 61.1 & 70.6 & 82.3 & 88.5 & 91.4 & 93.0 & 93.8 \\
  & NC-SSM (7.4K)          & 95.3 & 61.4 & 69.2 & 77.8 & 86.8 & 90.1 & 92.4 & 92.9 \\
...
\multirow{5}{*}{White}
  ...
  & BC-ResNet-1 (7.5K)     & 95.3 & \textbf{58.6} & \textbf{64.6} & \textbf{76.7} & 85.9 & 90.2 & 92.6 & 93.8 \\
  & NM-Matched (7.4K)      & 95.1 & 31.1 & 59.1 & 75.0 & 85.5 & 89.8 & 92.8 & 93.9 \\
  & NC-SSM (7.4K)          & 95.3 & 50.8 & 63.0 & 75.5 & 85.2 & 90.0 & 92.5 & 93.5 \\
...
\multirow{5}{*}{Babble}
  ...
  & BC-ResNet-1 (7.5K)     & 95.3 & \textbf{76.7} & \textbf{84.3} & \textbf{88.8} & \textbf{92.1} & \textbf{93.8} & \textbf{94.4} & 95.0 \\
...
\multirow{5}{*}{Pink}
  ...
  & BC-ResNet-1 (7.5K)     & 95.3 & \textbf{58.9} & \textbf{66.0} & \textbf{79.7} & \textbf{88.4} & 91.8 & \textbf{93.8} & 94.5 \\
...
\end{tikzpicture}
\caption{Accuracy vs.\ SNR per noise type. NC-SSM-Large ($\blacklozenge$) matches BC-ResNet-1 ($\blacksquare$) above 5\,dB with 4.1$\times$ fewer MACs. The gap below $-5$\,dB reflects $O(\sigma_n^6)$ multiplicative noise propagation. Babble shows the smallest gap due to NC-SSM's non-stationary adaptation.}
\label{fig:snr_curve}
\end{figure*}
...
Crucially, this gap is \emph{not} a fundamental theoretical limitation.
...
NC-SSM's selectivity gate enables this transition, but in practice, residual selectivity at extreme noise ($\sigma \approx 0.05$ at $-15$\,dB) still introduces multiplicative coupling: even 1\% selectivity amplifies noise by $\sigma_n^4 \approx 1{,}000\times$ at $-15$\,dB.

\smallskip\noindent\textbf{Noise-Aware Selective Gating (NASG).}
We propose a principled solution: \emph{scale the projection input by an SNR-dependent weight} before computing selective parameters.
Let $g = \sigma(\alpha \cdot \overline{\text{SNR}} + \beta)$ be a learnable gate with steep initialization ($\alpha{=}20$, $\beta{=}{-5}$).
The selective parameters become:
\begin{equation}
\Delta_t = \sigma_\Delta \cdot g \cdot \Delta_{\text{sel}}(x_t) + (1-\sigma_\Delta) \cdot \Delta_{\text{base}},
\label{eq:nasg}
\end{equation}
and similarly for $\mathbf{B}_t$, $\mathbf{C}_t$.
At low SNR, $g \to 0$: selective components vanish, yielding pure LTI mode with $O(\sigma_n^2)$.
At high SNR, $g \to 1$: full selectivity is preserved.
The effective selectivity $\sigma \cdot g$ at $-15$\,dB reduces to $\approx 0.001$, suppressing cross-term noise by $\sim$2{,}500$\times$ compared to the $\sigma$-gate alone---with only 2 additional parameters per block.

\smallskip\noindent\textbf{Adaptive State Dimensionality Reduction.}
While NASG suppresses the \emph{selective} component, the fixed parameters $\Delta_{\text{base}}$, $\mathbf{B}_{\text{base}}$ still drive the state update in LTI mode.
At $-15$\,dB, noise accumulates across all $N$ state dimensions, with higher-indexed states (longer effective memory via $|\bar{A}_n| \to 1$) accumulating proportionally more noise.
We introduce \emph{input-gated state masking}: a learnable per-state gate $\mathbf{m} \in \mathbb{R}^N$ that scales the state \emph{input}:
\begin{equation}
\mathbf{h}_t = \bar{\mathbf{A}}_t\,\mathbf{h}_{t-1} + \mathbf{m} \odot \bar{\mathbf{B}}_t\,x_t,
\label{eq:state_mask}
\end{equation}
where $m_n = \sigma(\gamma \cdot \overline{\text{SNR}} + \beta_0 - p_n)$ with linearly increasing penalty $p_n = 2n/(N{-}1)$ ($\gamma{=}8$, $\beta_0{=}{-}2$; 2 additional parameters per block).
At $-15$\,dB white noise, $m_0 \approx 0.15$ and $m_7 \approx 0.02$, yielding an effective state dimension $N_{\text{eff}} \approx 4\text{--}5$ (from 8).

Crucially, the mask is applied to the state \emph{input} rather than the output readout ($y_t = (\mathbf{m} \odot \mathbf{C})\,\mathbf{h}_t$).
Output-gating allows noise to accumulate freely across $T$ frames before discarding 97\% of the corrupted state; input-gating prevents noise \emph{entry}, keeping $\|\mathbf{h}_{t,n}\| \approx 0$ for masked states.
...
\smallskip\noindent\textbf{Low-SNR performance gap.}
While NC-SSM-Large matches or exceeds BC-ResNet-1 above $-5$\,dB SNR, a significant accuracy gap persists at extreme noise levels (e.g., white noise at $-15$\,dB: 34.1\% vs.\ 58.6\%).
This directly reflects the $O(\sigma_n^6)$ vs.\ $O(\sigma_n^2)$ variance gap analyzed in Section~\ref{sec:mult_noise}.
The NASG mechanism proposed in Section~\ref{sec:discussion} addresses this by suppressing the selective component at low SNR (effective selectivity $\sigma \cdot g \approx 0.001$ at $-15$\,dB), theoretically reducing noise cross-terms by $\sim$2{,}500$\times$.
Further directions include explicit SNR-conditioned mixture of experts within the SSM block and the general \emph{selectivity--noise disentanglement} framework applicable to any input-dependent architecture.
...
NC-SSM resolves this through per-sub-band selectivity modulation, Learned Spectral Gate, and optimized spectral subtraction with adaptive bypass, achieving \textbf{7{,}443 parameters}---21 fewer than BC-ResNet-1 (7{,}464)---while matching its clean accuracy (95.3\%) with \textbf{5.5$\times$ fewer MACs} and \textbf{1.8\,ms latency} on Cortex-M7.
Scaling to NC-SSM-Large (10{,}201 parameters) further improves clean accuracy to \textbf{95.6\%}---surpassing BC-ResNet-1---while maintaining \textbf{4.1$\times$ fewer MACs}, \textbf{2.4\,ms latency}, and \textbf{2.7$\times$ less RAM}.
