...
A promising direction is \emph{decoupling} the parameter computation from the noisy input---computing $\Delta_t$, $\mathbf{B}_t$ from a temporally smoothed signal while preserving the original input for state updates---which would break the cubic noise coupling at its source.
...
This directly reflects the $O(\sigma_n^6)$ vs.\ $O(\sigma_n^2)$ variance gap analyzed in Section~\ref{sec:mult_noise}: CNN fixed kernels absorb noise statistics efficiently through spatial averaging, while SSM state updates amplify noise multiplicatively.
NC-SSM's per-sub-band selectivity gate already provides \emph{implicit} mixture-of-experts behavior---blending selective and LTI modes per frequency sub-band.
A promising direction is \textbf{explicit SNR-conditioned mixture of experts} within the SSM block, where structurally different expert pathways (full selective vs.\ noise-hardened LTI with reduced state dimension) are routed by estimated SNR, potentially closing the low-SNR gap while preserving the computational efficiency advantage.
